package com.neil.api.gateway.utils.currency;
...
public class NumberToChineseCapitalUtils {

// 基本考虑
// 人民币大写单位银行规定用“元”，无零头金额后跟“整”，有则不跟
...
// （十进位处于第一位不发壹音，如“拾元”非“壹拾元”，十进位处前有零是否不发壹音不太确定，如“叁仟零壹拾元”还是“叁仟零拾元”？）
...
// 因为至少要加添两个汉字并且改动后数字必须进位才能窜改成，（如“拾万”可改成“叁拾万”，而“壹拾万”至少要改成“壹佰壹拾万”）
  ...
  public static void main(String args[]) {
    NumberToChineseCapitalUtils mon = new NumberToChineseCapitalUtils();
    System.out.println(0.99 + mon.numToCnStr(0.99));
    System.out.println(999999 + mon.numToCnStr(999999));
//    System.out.println(-99999999999999.999 + mon.numToCnStr(-99999999999999.999));
//    System.out.println(99999999999999.999 + mon.numToCnStr(99999999999999.999));
//    System.out.println(mon.numToCnStr(100000000.00));
//    System.out.println(mon.numToCnStr(100000001.00));
  }

  private String[] cnDigiStr = new String[]{"零", "壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖"};

  private String[] cnDiviStr = new String[]{"", "拾", "佰", "仟", "万", "拾", "佰", "仟", "亿",
      "拾", "佰", "仟", "万", "拾", "佰", "仟", "亿",
      "拾", "佰", "仟", "万", "拾", "佰", "仟"};
   ...
   */
  private String PositiveIntegerToCnStr(String NumStr) {
    String cnStr = "";  //实际的中文金额
    ...
    for (int i = len - 1; i >= 0; i--) {
      ...
      if (n != 0) {
        if (isLastZero) cnStr += cnDigiStr[0];  // 若干零后若跟非零值，只显示一个零
        // 除了亿万前的零不带到后面
        //if( !( n==1 && (i%4)==1 && (isLastZero || i==len-1) ) )    // 如十进位前有零也不发壹音用此行
        if (!(n == 1 && (i % 4) == 1 && i == len - 1))     // 十进位处于第一位不发壹音
          cnStr += cnDigiStr[n];
        cnStr += cnDiviStr[i];    // 非零值后加进位，个位为空
        hasValue = true;   // 置万进位前有值标记
      }
      else {
        if ((i % 8) == 0 || ((i % 8) == 4 && hasValue))  // 亿万之间必须有非零值方显示万
          cnStr += cnDiviStr[i];   // “亿”或“万”
      }
   ...
   * @return String 大写文中
   */
  public String numToCnStr(double val) {
    String SignStr = "";
    String TailStr;
